...
class SnippetController {
  ...
  static async create(req, res) {
    try {
      ...
      const id = await SnippetModel.create({ user_id: req.user.id, title, language, source_code, is_public });
  ...
  static async getById(req, res) {
    try {
      const snippet = await SnippetModel.findById(req.params.id, req.user.id);
  ...
  static async getPublic(req, res) {
    try {
      const snippets = await SnippetModel.getPublic(req.user.id, 20);
